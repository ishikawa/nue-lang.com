 ...
 * @param {number | null} [max_steps]
 * @param {number | null} [max_output_lines]
 * @param {number | null} [max_memory_bytes]
 * @returns {any}
 */
export function run_playground(source, timeout_ms, max_steps, max_output_lines, max_memory_bytes) {
    const ptr0 = passStringToWasm0(source, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.run_playground(ptr0, len0, isLikeNone(timeout_ms) ? 0x100000001 : (timeout_ms) >>> 0, isLikeNone(max_steps) ? 0x100000001 : (max_steps) >>> 0, isLikeNone(max_output_lines) ? 0x100000001 : (max_output_lines) >>> 0, isLikeNone(max_memory_bytes) ? 0x100000001 : (max_memory_bytes) >>> 0);
    return ret;
}
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('nue_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
